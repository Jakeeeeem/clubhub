...
// Returns all children with their club information for the family switcher
router.get("/family", authenticateToken, async (req, res) => {
  try {
    ...
    console.log(
      `📋 Family members for user ${req.user.id}: ${playersWithAge.length} found`,
    );
    ...
    try {
      ...
      if (!playerUserId && email) {
        ...
        if (userResult.rows.length > 0) {
          playerUserId = userResult.rows[0].id;
          console.log(
            `🔗 Linked player to existing user account during update: ${email}`,
          );
        }
      }
...
// 🔥 TOP-LEVEL ROUTE: Manual Acceptance Route
router.post(
  "/:id/accept",
  authenticateToken,
  requireOrganization,
    ...
    try {
      ...
      if (!isAuthorized) {
        return res.status(403).json({
          error: "Access denied",
          message:
            "You do not have permission to delete players from this club",
        });
      }
      ...
      await withTransaction(async (client) => {
        // Delete player
        await client.query("DELETE FROM players WHERE id = $1", [
          req.params.id,
        ]);
  ...
  async (req, res) => {
    try {
      ...
      if (check.rows.length === 0) {
        return res
          .status(404)
          .json({ error: "Child profile not found or access denied" });
      }
...
router.post("/:id/history", authenticateToken, async (req, res) => {
  try {
    const playerId = req.params.id;
    const { club_name, team_name, start_date, end_date, achievements } =
      req.body;
...
router.delete("/history/:historyId", authenticateToken, async (req, res) => {
  try {
    ...
    await query("DELETE FROM player_history WHERE id = $1", [
...
router.post(
  ...
  async (req, res) => {
    try {
      const { teamName, clubName, startDate, endDate, position, achievements } =
        req.body;
