...
const staffValidation = [
  ...
  body('role').isIn(['coach', 'assistant-coach', 'treasurer', 'coaching-supervisor', 'referee', 'administrator']).withMessage('Invalid role'),
...
router.get('/:id/performance', authenticateToken, async (req, res) => {
  try {
    ...
    const hasPermission = req.user.id === staff.user_id || 
                         req.user.accountType === 'organization';
    ...
    if (!['coach', 'assistant-coach', 'coaching-supervisor'].includes(staff.role)) {
    ...
    const overallStats = teamPerformanceResult.rows.reduce((acc, team) => {
    ...
    overallStats.win_percentage = overallStats.total_games > 0 
...
// Update staff role
router.patch('/:id/role', authenticateToken, requireOrganization, [
  body('role').isIn(['coach', 'assistant-coach', 'treasurer', 'coaching-supervisor', 'referee', 'administrator']).withMessage('Invalid role')
], async (req, res) => {
  try {
...
router.post('/', authenticateToken, requireOrganization, staffValidation, async (req, res) => {
  try {
    const errors = validationResult(req);
    if (!errors.isEmpty()) {
      return res.status(400).json({
...
// Update staff member
router.put('/:id', authenticateToken, requireOrganization, staffValidation, async (req, res) => {
  try {
    const errors = validationResult(req);
    if (!errors.isEmpty()) {
      return res.status(400).json({
...
router.patch('/:id/permissions', authenticateToken, requireOrganization, [
  ...
  try {
    ...
    const validPermissions = ['finances', 'players', 'events', 'listings', 'reports', 'settings'];
