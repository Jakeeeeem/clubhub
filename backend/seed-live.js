...
async function seedLive() {
  ...
  try {
    ...
    // CLEANUP PREVIOUS SEED DATA (To prevent duplicates)
    ...
    await client.query(
      "DELETE FROM organization_members WHERE user_id IN (SELECT id FROM users WHERE email IN ($1, $2))",
      ["demo-admin@clubhub.com", "multi@demo.com"],
    );
    ...
    const createProducts = async (clubId, prefix) => {
      console.log(
        `   🛒 Seeding Products for Club ${clubId.substring(0, 8)}...`,
      );
      const products = [
        {
          name: `${prefix} Home Kit`,
          price: 49.99,
          cat: "kit",
          img: "https://placehold.co/400x400/000000/FFF?text=Home+Kit",
        },
        {
          name: `${prefix} Away Kit`,
          price: 49.99,
          cat: "kit",
          img: "https://placehold.co/400x400/cccccc/000?text=Away+Kit",
        },
        {
          name: `${prefix} Training Top`,
          price: 29.99,
          cat: "training",
          img: "https://placehold.co/400x400/blue/white?text=Training",
        },
        {
          name: `${prefix} Hoodie`,
          price: 39.99,
          cat: "merch",
          img: "https://placehold.co/400x400/red/white?text=Hoodie",
        },
        {
          name: `${prefix} Scarf`,
          price: 15.0,
          cat: "merch",
          img: "https://placehold.co/400x400/green/white?text=Scarf",
        },
        {
          name: `${prefix} Water Bottle`,
          price: 10.0,
          cat: "equipment",
          img: "https://placehold.co/400x400/yellow/black?text=Bottle",
        },
      ];
    ...
    const createEvents = async (clubId, clubName) => {
      console.log(`   📅 Seeding Events for ${clubName}...`);
      const eventTypes = ["training", "match", "social", "tournament"];
      const titles = [
        `Weekly Training - ${clubName}`,
        `League Match: ${clubName} vs Rivals`,
        `Team Dinner - ${clubName}`,
        `Regional Cup (${clubName})`,
      ];
    ...
    const createTeams = async (clubId, clubName) => {
      console.log(`   ⚽ Seeding Teams for ${clubName}...`);
      const teamNames = [
        `U12 ${clubName} Lions`,
        `U14 ${clubName} Tigers`,
        `First Team`,
      ...
      for (const name of teamNames) {
        const res = await client.query(
          `INSERT INTO teams (club_id, name, sport, age_group, description, created_at)
           VALUES ($1, $2, 'Football', 'Various', $3, NOW()) RETURNING id`,
          [clubId, name, `Official team for ${clubName}`],
        );
    ...
    const createPlans = async (clubId) => {
      const plans = [
        ...
        { name: "Silver", price: 85.0 },
      ];
      for (const p of plans) {
        await client.query(
          `INSERT INTO plans (club_id, name, price, interval, description, active)
           VALUES ($1, $2, $3, 'month', 'Membership Plan', true)`,
          [clubId, p.name, p.price],
        );
      }
    };

    const createMemberPayments = async (playerId, clubId, count = 2) => {
      for (let i = 0; i < count; i++) {
        ...
        await client.query(
          ...
          [
            ...
            i === 0 ? "pending" : "paid",
            date,
            `Membership Fee - ${date.toLocaleString("default", { month: "long" })}`,
          ],
        );
      }
    };
      ...
      for (const l of listingsData) {
        const newListing = await client.query(
          ...
          [
            ...
            l.type,
            l.pos,
            `Open role for ${l.title} at ${clubName}`,
          ],
        );
        ...
        const applicants = [
          {
            first: "John",
            last: "Doe",
            email: `john.doe.${Math.random().toString(36).substring(7)}@test.com`,
            pos: "Forward",
          },
          {
            first: "Jane",
            last: "Smith",
            email: `jane.smith.${Math.random().toString(36).substring(7)}@test.com`,
            pos: "Goalkeeper",
          },
          {
            first: "Mike",
            last: "Johnson",
            email: `mike.j.${Math.random().toString(36).substring(7)}@test.com`,
            pos: "Coach",
          },
        ];
      ...
      const staffMembers = [
        {
          ...
          last: "Coachman",
          role: "coach",
          email: `alex.coach.${Math.random().toString(36).substring(7)}@test.com`,
        },
        {
          first: "Pep",
          last: "Tactics",
          role: "assistant-coach",
          email: `pep.tactics.${Math.random().toString(36).substring(7)}@test.com`,
        },
        {
          first: "Jurgen",
          last: "Press",
          role: "coach",
          email: `jurgen.press.${Math.random().toString(36).substring(7)}@test.com`,
        },
      ];
    ...
    const orgA = await getOrCreateOrg(
      "Elite Pro Academy",
      "elite-pro-academy",
      "Premium Training",
      "Football",
    ...
    const teamsB = await createTeams(orgB, "Sunday League");
    ...
    console.log(
      "👤 Adding Owner as Player to both clubs for easier testing...",
    );

    // Owner -> Player record in Elite Pro
    const ownerPlayerA = await client.query(
      `INSERT INTO players (user_id, club_id, first_name, last_name, email, position, payment_status, date_of_birth)
       VALUES ($1, $2, 'Demo', 'Admin', $3, 'Striker', 'paid', '1990-01-01') RETURNING id`,
      [ownerId, orgA, ownerEmail],
    );

    // Owner -> Player record in Sunday League
    const ownerPlayerB = await client.query(
      `INSERT INTO players (user_id, club_id, first_name, last_name, email, position, payment_status, date_of_birth)
       VALUES ($1, $2, 'Demo', 'Admin', $3, 'Goalkeeper', 'paid', '1990-01-01') RETURNING id`,
      [ownerId, orgB, ownerEmail],
    );

    // Assign Demo Admin to teams
    await client.query(
      `INSERT INTO team_players (team_id, player_id) VALUES ($1, $2)`,
      [teamsA[2], ownerPlayerA.rows[0].id],
    ); // First Team
    await client.query(
      `INSERT INTO team_players (team_id, player_id) VALUES ($1, $2)`,
      [teamsB[2], ownerPlayerB.rows[0].id],
    ); // First Team

    await createMemberPayments(ownerPlayerA.rows[0].id, orgA, 3);
    await createMemberPayments(ownerPlayerB.rows[0].id, orgB, 1);

    // Ensure Owner role in both clubs
    await client.query(
      `
            INSERT INTO organization_members (user_id, organization_id, role, status)
    ...
    // --- 5. ADD CHILDREN FOR DEMO ADMIN ---
    console.log("👤 Seeding Child Profiles for Demo Admin...");
    ...
    await client.query(
      `INSERT INTO team_players (team_id, player_id) VALUES ($1, $2)`,
      [teamsA[0], leoPlayer.rows[0].id],
    ); // U12 Lions
    await createMemberPayments(leoPlayer.rows[0].id, orgA, 5);
    ...
    await client.query(
      `INSERT INTO team_players (team_id, player_id) VALUES ($1, $2)`,
      [teamsB[1], miaPlayer.rows[0].id],
    ); // U14 Tigers
    await createMemberPayments(miaPlayer.rows[0].id, orgB, 2);

    await client.query("COMMIT");
    console.log(
      "✅ SEEDING COMPLETE! Login with demo-admin@clubhub.com or multi@demo.com (password123)",
    );
  } catch (e) {
