/* =========================================================
        ...
        /* =========================================================
           API helpers
           ========================================================= */
        const API = (typeof window !== 'undefined' && window.API_BASE) || ((typeof window !== 'undefined' && window.location.hostname === 'localhost') ? 'http://localhost:3000' : '');
        ...
        async function loadMyForms() {
            try {
                ...
                if (!forms.length) {
                    container.innerHTML = '<p style="color:var(--text-muted); font-size:0.85rem;">No forms yet. Click + New Form to start.</p>';
                    return;
                }
        ...
        `).join('');
                ...
                document.getElementById('myFormsContainer').innerHTML = `<p style="color:#f87171; font-size:0.85rem;">Failed to load forms</p>`;
            }
        }
        ...
        <div class="field-row ${selectedFieldIdx === i ? 'selected' : ''}"
            ...
            <div class="field-row-info">
                <div class="field-row-label">${f.label || 'Untitled field'}</div>
                <div class="field-row-type">${f.type}</div>
            </div>
            ${f.required ? '<span class="field-row-required">Required</span>' : ''}
            <button class="field-row-delete" onclick="event.stopPropagation(); deleteField(${i})">🗑</button>
        </div>
    `).join('');
        }
        ...
        function addField(type) {
            const labels = { text: 'Text Answer', number: 'Number', email: 'Email', phone: 'Phone', date: 'Date', textarea: 'Paragraph', select: 'Dropdown', multiselect: 'Multi-select', checkbox: 'Checkbox', file: 'File Upload' };
            fields.push({ label: labels[type] || 'Field', type, required: false, options: ['select', 'multiselect', 'checkbox'].includes(type) ? ['Option 1'] : [] });
        ...
        function deleteField(idx) {
            ...
            renderCanvas();
            if (selectedFieldIdx >= 0) renderProps(selectedFieldIdx); else clearProps();
        }
    ...
    `;
        ...
        function clearProps() {
            document.getElementById('propsContent').innerHTML = '<div class="props-placeholder">🖱 Click a field on the canvas to edit its properties</div>';
        }
        ...
        function updateField(idx, key, value) {
            ...
            if (key === 'label') {
                ...
                if (rows[idx]) rows[idx].querySelector('.field-row-label').textContent = value || 'Untitled field';
            }
        ...
        function initDnD() {
            ...
            canvas.addEventListener('drop', e => {
                ...
                if (dragSrcType) { addField(dragSrcType); dragSrcType = null; }
            });
        }

        /* ── Reordering rows ── */
        let dragRowSrc = -1;
        function onRowDragStart(e, idx) { dragRowSrc = idx; dragSrcType = null; e.dataTransfer.effectAllowed = 'move'; }
        ...
        async function saveForm() {
            ...
            try {
                if (currentFormId) {
                    ...
                    await apiFetch(`/api/forms/${currentFormId}`, { method: 'PUT', body: JSON.stringify({ title: payload.title, description: payload.description }) });
                    showToast('✅ Form updated!');
                } else {
                    const { form } = await apiFetch('/api/forms', { method: 'POST', body: JSON.stringify(payload) });
    ...
    `).join('') || '<p style="color:var(--text-muted)">No fields yet.</p>';
        ...
        function renderPreviewInput(f) {
            switch (f.type) {
                case 'textarea': return `<textarea rows="3" placeholder="${f.hint || ''}"></textarea>`;
                case 'select': return `<select><option value="">Choose...</option>${(f.options || []).map(o => `<option>${o}</option>`).join('')}</select>`;
                case 'multiselect': return (f.options || []).map(o => `<label style="display:flex;gap:.5rem;align-items:center;"><input type="checkbox"> ${o}</label>`).join('');
                case 'checkbox': return (f.options || []).map(o => `<label style="display:flex;gap:.5rem;align-items:center;"><input type="checkbox"> ${o}</label>`).join('');
                case 'date': return `<input type="date">`;
                case 'number': return `<input type="number" placeholder="${f.hint || ''}">`;
                case 'email': return `<input type="email" placeholder="${f.hint || 'your@email.com'}">`;
                case 'phone': return `<input type="tel" placeholder="${f.hint || '+44...'}">`;
                case 'file': return `<input type="file">`;
                default: return `<input type="text" placeholder="${f.hint || ''}">`;
            }
        }
        ...
        async function viewResponses() {
            ...
            try {
                const { responses, total } = await apiFetch(`/api/forms/${currentFormId}/responses`);
                ...
                win.document.write(`<html><head><title>Form Responses</title><style>body{font-family:system-ui;padding:2rem;background:#111;color:#eee;}table{border-collapse:collapse;width:100%}th,td{border:1px solid #333;padding:.5rem .75rem;text-align:left}th{background:#1e1e2e}</style></head><body>`);
                win.document.write(`<h2>📊 Responses — ${total} total</h2><table><thead><tr><th>Submitted</th><th>User</th>${headers.map(h => `<th>${h}</th>`).join('')}</tr></thead><tbody>`);
                responses.forEach(r => {
                    win.document.write(`<tr><td>${new Date(r.submitted_at).toLocaleString()}</td><td>${r.first_name ? r.first_name + ' ' + r.last_name : 'Guest'}</td>${headers.map(h => `<td>${r.response_data[h] ?? ''}</td>`).join('')}</tr>`);
                });
