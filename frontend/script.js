    ...
    return new Promise((resolve, reject) => {
        function checkApiService() {
            if (typeof apiService !== 'undefined') {
                ...
                console.log(`⏳ Waiting for API Service... (${initializationAttempts}/${maxInitializationAttempts})`);
...
function checkAuthStateWithoutAPI() {
    ...
    if (storedUser && storedUserType && token) {
        try {
            ...
            console.log('✅ User loaded from localStorage:', AppState.currentUser.email);
        } catch (parseError) {
...
async function handleRegister(e) {
    ...
    try {
        ...
        if (accountType === 'organization') {
            const checkboxes = document.querySelectorAll('input[name="orgType"]:checked');
            orgTypes = Array.from(checkboxes).map(cb => cb.value);
            
            if (orgTypes.length === 0) {
                showNotification('Please select at least one organization type', 'error');
                return;
            }
        }
...
async function loadInitialData() {
    ...
    if (typeof apiService === 'undefined') {
        ...
        showNotification('Some features may not work - API service unavailable', 'info');
        return;
    }
...
async function loadUserData() {
    ...
    try {
        ...
        const [clubs, events, playersResponse, staff, teams] = await Promise.all([
            apiService.getClubs().catch(e => { console.warn('Failed to load clubs:', e); return []; }),
            apiService.getEvents().catch(e => { console.warn('Failed to load events:', e); return []; }),
            apiService.getPlayers().catch(e => { console.warn('Failed to load players:', e); return { players: [] }; }),
            apiService.getStaff().catch(e => { console.warn('Failed to load staff:', e); return []; }),
            apiService.getTeams().catch(e => { console.warn('Failed to load teams:', e); return []; })
        ]);
        ...
        const currentUserAsPlayer = AppState.players.find(p => p.email === AppState.currentUser?.email);
        if (currentUserAsPlayer) {
            console.log('✅ Current user found as player:', currentUserAsPlayer.first_name, currentUserAsPlayer.last_name);
        } else {
            console.log('⚠️ Current user not found in players list');
            console.log('📧 Current user email:', AppState.currentUser?.email);
            console.log('📧 Available player emails:', AppState.players.map(p => p.email));
        }
